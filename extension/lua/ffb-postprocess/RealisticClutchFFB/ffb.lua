...
function script.update(ffb, dt)
  ...
  if hasSharedIPC then
    ...
    -- 1. DRIVELINE MACRO-JUDDER & BITE ZONE RESONANCE (11.5 - 13.0 Hz Resonant Mode)
    if isSlipping and gainJudder > 0.001 then
      local biteCenter = sharedData.clutchBitePosition or 0.45
      local distFromBite = math.abs(engagement - biteCenter) / 0.30
      local biteZone = clamp(1.0 - distFromBite, 0.0, 1.0)
      local smoothBite = biteZone * biteZone * (3.0 - 2.0 * biteZone)

      -- Frequência tátil de máxima percepção para motores de volante DirectInput (12.0 Hz)
      local biteFreq = 12.0
      judderPhase = (judderPhase + biteFreq * dt) % 1.0

      local slipWeight = clamp(slipRpm / 150.0, 0.40, 1.0)
      local judderWave = math.sin(judderPhase * 2.0 * math.pi)

      -- Amplitude sólida de até 0.28 (28% FFB) no ápice do ponto de embreagem:
      -- Firme e nítido nas mãos no PCYES W270 / Logitech / Thrustmaster (média rigorosamente nula)
      local biteHaptic = judderWave * (0.10 + 0.18 * smoothBite) * slipWeight * gainJudder
      extraForce = extraForce + biteHaptic
    else
      judderPhase = 0.0
    end

    -- 2. FRICTION MICRO-CHATTER (22 - 34 Hz Textura Tátil de Fricção)
    if isSlipping and slipRpm > 10.0 and gainChatter > 0.001 then
      local chatterFreq = clamp(22.0 + 10.0 * (slipRpm / 1200.0), 22.0, 34.0)
      ...
      extraForce = extraForce + (chatterWave * normalClamp * velocityWeight * 0.045 * gainChatter)
    else
      chatterPhase = 0.0
    end
    ...
    if rpm > 280.0 and engagement > 0.05 and gainCombustion > 0.001 then
      ...
      extraForce = extraForce + (wave * throttleScalar * engagement * 0.040 * gainCombustion)
    else
      combustionPhase = 0.0
    end
    ...
    if sharedData.isStalling and gainLugging > 0.001 then
      ...
      extraForce = extraForce + (lugWave * (lugDeficit ^ 1.2) * 0.12 * gainLugging)
    else
      luggingPhase = 0.0
    end
    ...
    if recoilTimer > 0.0 then
      ...
      local recoilShock = recoilAmplitude * math.exp(-22.0 * t) * math.sin(45.0 * t) * 0.15 * gainLugging
  ...
  -- =========================================================================
    ...
    if rpm > 220.0 then
      ...
      extraForce = extraForce + (wave * (config.idleGain or 0.25) * idleAtten * 0.040)
    end

    -- Bite Zone Chatter & Tactile Resonance (12.0 Hz)
    if gear ~= 0 and clutchPedal > 0.18 and clutchPedal < 0.82 and rpm > 220.0 and speedKmh < 24.0 then
      ...
      extraForce = extraForce + (chatterWave * (config.biteGain or 1.2) * (0.10 + 0.18 * smoothBite))
    end

    -- Pre-stall Lugging
    if gear ~= 0 and rpm > 180.0 and rpm < 580.0 and clutchPedal > 0.28 then
      luggingPhase = (luggingPhase + 5.5 * dt) % 1.0
      local lugWave = math.sin(luggingPhase * 2.0 * math.pi)
      local severity = clamp((580.0 - rpm) / 380.0, 0.0, 1.0)
      extraForce = extraForce + (lugWave * (config.stallGain or 0.90) * severity * 0.12)
    end

    -- Stall recoil
    if recoilTimer > 0.0 then
      local t = 0.16 - recoilTimer
      local recoilShock = recoilAmplitude * math.exp(-22.0 * t) * math.sin(45.0 * t) * 0.15
      extraForce = extraForce + recoilShock
      recoilTimer = math.max(0.0, recoilTimer - dt)
    end
  end

  -- HARD SAFE CLAMP: Max +/- 0.32 (32% FFB)
  -- Permite amplitude firme e nítida para volantes de engrenagem/correia (PCYES W270 / Logitech / Thrustmaster)
  extraForce = clamp(extraForce, -0.32, 0.32)
